...
PDF export is implemented through the protected `GET /api/exam-templates/{template}/pdf` endpoint. Administrators and teachers may download any template they can manage; students and parents may download only published templates. The Laravel service renders a self-contained RTL Blade document through TCPDF’s Unicode/RTL writer, returns `application/pdf`, marks the response `private, no-store`, and uses an attachment filename derived from the exam title. This avoids the previous Dompdf Arabic glyph-order/shaping failure. The client downloads the returned blob directly, so users do not need the browser print dialog. Multiple-choice options use empty checkbox controls for student marking; the preview retains browser print as a fallback only when no server export callback is supplied.
...
The `ExamPaper` component has a server-rendered regression test covering Arabic metadata, watermark text, ordered questions, empty option checkboxes, and essay answer space. Laravel feature tests cover staff PDF generation, PDF signature and headers, published-only student access, and existing monitored-session behavior. The regenerated PDF was visually inspected and text-extracted: Arabic glyphs are connected and readable, and options render as empty squares. The frontend suite passes with 18 tests, TypeScript passes, and the production build completes. The preview overlay uses a responsive one-column layout below 700px, wrapped toolbar controls, scrollable paper content, checkbox controls, strengthened question cards, and repeated watermark layers across the paper and individual questions.
